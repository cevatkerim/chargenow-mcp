...
async function getCoordinates(address, config) {
...
async function getChargePools(lat, lon, config) {
...
async function getChargePointStatuses(chargePointIds) {
...
async function getPoolDetails(poolIds) {
    if (poolIds.length === 0) {
        return [];
    }
    const body = JSON.stringify({
...
function formatResults(statuses, pools, poolDetails, searchCoords, address) {
    ...
    // Process each pool
    pools.forEach(pool => {
        const distance = calculateDistance(searchCoords.lat, searchCoords.lon, pool.latitude, pool.longitude);
        const poolStatuses = statuses.filter(status => pool.chargePoints.some(cp => cp.id === status.dcsChargePointId));
    ...
    if (totalOffline > 0)
        summary += `• ${totalOffline} charge points OFFLINE\n`;
    summary += `\nDetailed Locations:\n`;
    ...
    sortedPools.forEach(({ pool, detail, statuses: poolStatuses, distance }) => {
        ...
        if (chargingCount > 0)
            summary += `   • ${chargingCount} points in use\n`;
        if (offlineCount > 0)
            summary += `   • ${offlineCount} points offline\n`;
...
server.tool('find_available_chargepoints', 'Finds available electric vehicle charge points near a given address (street and city).', {
